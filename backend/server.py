...
def _request_ip(request: Request) -> str:
    # Do not trust a caller-supplied forwarding header for authentication
    # throttling. The email is also part of the hashed rate-limit identifier.
...
async def save_lulu_credentials(payload: LuluCredentialsUpdate, current_user: User = Depends(get_current_user)):
    _require_super_admin(current_user)
    ...
    await db.integration_credentials.update_one(
        {"provider": "lulu", "owner_id": current_user.id},
        {"$set": {
            "provider": "lulu", "owner_id": current_user.id, "environment": payload.environment,
            "client_key_encrypted": encrypt_credential(client_key),
            "client_secret_encrypted": encrypt_credential(client_secret),
            "client_key_hint": credential_hint(client_key), "connection_status": "not_tested",
            "updated_at": now,
...
async def clear_lulu_credentials(current_user: User = Depends(get_current_user)):
    _require_super_admin(current_user)
    await db.integration_credentials.delete_one({"provider": "lulu", "owner_id": current_user.id})
...
async def test_lulu_credentials(current_user: User = Depends(get_current_user)):
    ...
    except (KeyError, LuluIntegrationError) as exc:
        detail = str(exc) if isinstance(exc, LuluIntegrationError) else "Stored Lulu credentials are incomplete."
        await db.integration_credentials.update_one(
            {"provider": "lulu", "owner_id": current_user.id},
            {"$set": {"connection_status": "error", "last_checked_at": now, "last_connection_error": detail}},
        )
    ...
    return LuluConnectionResponse(
        connected=True, environment=result["environment"],
